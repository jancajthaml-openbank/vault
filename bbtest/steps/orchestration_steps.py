...
def step_impl(context, package, operation):
  if operation == 'installed':
    ...
    assert os.path.isfile('/etc/vault/conf.d/init.conf') is True, 'config file does not exists'
...
def offboard_unit(context, tenant):
  ...
  (code, result, error) = Shell.run(['journalctl', '-o', 'cat', '-u', 'vault-unit@{}.service'.format(tenant), '--no-pager'])
...
def onboard_unit(context, tenant):
  (code, result, error) = Shell.run(["systemctl", 'enable', 'vault-unit@{}'.format(tenant)])
  assert code == 'OK', str(code) + ' ' + str(result) + ' ' + str(error)
  (code, result, error) = Shell.run(["systemctl", 'start', 'vault-unit@{}'.format(tenant)])
  assert code == 'OK', str(code) + ' ' + str(result) + ' ' + str(error)
