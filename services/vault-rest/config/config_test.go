...
func TestGetConfig(t *testing.T) {
  ...
  t.Log("has defaults for all values")
  {
    ...
    if config.ServerPort != 4400 {
      t.Errorf("ServerPort default value is not 4400")
    }
    if config.ServerKey != "" {
    ...
    if config.MetricsOutput != "/tmp/vault-rest-metrics" {
      t.Errorf("MetricsOutput default value is not /tmp/vault-rest-metrics")
    }
