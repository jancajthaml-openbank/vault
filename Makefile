...
bundle-binaries-%: %
	@docker-compose run --rm package --arch linux/$^ --pkg vault-rest --output /project/packaging/bin
	@docker-compose run --rm package --arch linux/$^ --pkg vault-unit --output /project/packaging/bin

.PHONY: bundle-debian-%
bundle-debian-%: %
	@docker-compose run --rm debian-package --version $(VERSION)+$(META) --arch $^ --pkg vault --source /project/packaging
...
.PHONY: test
test:
	@docker-compose run --rm test --pkg vault-rest --output /project/reports
	@docker-compose run --rm test --pkg vault-unit --output /project/reports
