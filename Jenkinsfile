...
pipeline {
    ...
    stages {
        ...
        stage('Setup') {
            steps {
                script {
                    ...
                    env.PROJECT_PATH = "${env.GOPATH}/src/github.com/jancajthaml-openbank/vault"

                    sh """
                        mkdir -p \
                            ${env.GOPATH}/src/github.com/jancajthaml-openbank && \
                        mv \
                            ${WORKSPACE}/services/vault-rest \
                            ${env.GOPATH}/src/github.com/jancajthaml-openbank/vault-rest
                        mv \
                            ${WORKSPACE}/services/vault-unit \
                            ${env.GOPATH}/src/github.com/jancajthaml-openbank/vault-unit
                    """
                }
            }
        }
        ...
        stage('Package Docker') {
            steps {
                script {
                    DOCKER_IMAGE_AMD64 = docker.build("openbank/vault:${env.GIT_COMMIT}", dockerOptions())
                }
            }
        }
